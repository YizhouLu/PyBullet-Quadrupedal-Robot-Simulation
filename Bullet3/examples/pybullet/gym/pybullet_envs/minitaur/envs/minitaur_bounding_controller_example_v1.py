...
for _ in range(1):
    ...
    flags.DEFINE_float("motor_kp", 1, "The position gain of the motor.")
    ...
    # INPUTS and INITIAL CONDITION
    mat1 = sio.loadmat('vel_07_Inputs_to_VS.mat')
    sorted(mat1.keys())
    Inputs_Joint_Position = mat1['int_joint_position']
    Inputs_Joint_Velocity = mat1['int_joint_velocity']
    ...
    mat3 = sio.loadmat('vel_07_InitialCondition_to_VS.mat')
    sorted(mat3.keys())
    IP_Torso_Position = mat3['init_pos']
    IP_Torso_Rotation = mat3['init_rot']
    IP_Torso_Velocity = mat3['init_vel']
    IP_Torso_AngularV = mat3['init_avel']
    IP_Joint_Position = mat3['init_joint']
    IP_Joint_Velocity = mat3['init_jvel']
    num_iter = range(len(Inputs_Joint_Position))
...
class Minitaur(object):
    ...
    def __init__(self,
        ...
        if self._accurate_motor_model_enabled:
            self._kp = motor_kp
            self._kd = motor_kd
            self._motor_model = Motor(torque_control_enabled=self._torque_control_enabled, kp=self._kp, kd=self._kd)
    ...
    def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
        ...
        """
        print('RESET Minitaur', 'reload urdf? ', reload_urdf, 'reset time: ', reset_time)
        if self._on_rack:
        ...
        if not self._torque_control_enabled and reset_time > 0.0:
            ...
            """
            for _ in range(100):
                self.ApplyAction([math.pi / 2] * self.num_motors, [0]*self.num_motors)
                self._pybullet_client.stepSimulation()
                self.ReceiveObservation()
            """
            if default_motor_angles is not None:
                ...
                for _ in range(num_steps_to_reset):
                    ...
                    self.ReceiveObservation()
        self.ReceiveObservation()
        print('Minitaur RESET', 'reload urdf? ', reload_urdf, 'reset time: ', reset_time)
    ...
    def ResetPose(self, add_constraint):
        ...
        print('Reset Leg Pose, add constraint? ',add_constraint)
    ...
    def _ResetPoseForLeg(self, leg_id, add_constraint):
        ...
        self._pybullet_client.resetJointState(self.quadruped,
            ...
            self._motor_direction[2 * leg_id] * IP_Joint_Position[4 * leg_id + 1],
        ...
        self._pybullet_client.resetJointState(self.quadruped,
            ...
            self._motor_direction[2 * leg_id ] * -(math.pi - IP_Joint_Position[4 * leg_id + 3]),
        ...
        if self._accurate_motor_model_enabled or self._pd_control_enabled:
            ...
            self._pybullet_client.setJointMotorControl2(
                bodyIndex = self.quadruped, jointIndex = self._joint_name_to_id["motor_" + leg_position + "R_joint"],
                controlMode = self._pybullet_client.POSITION_CONTROL, targetPosition = self._motor_direction[2 * leg_id] * IP_Joint_Position[4 * leg_id + 1],
                positionGain = self._kp, velocityGain = self._kd, force = self._max_force)
        self._pybullet_client.setJointMotorControl2(
    ...
    def GetBaseRollPitchYaw(self):
        ...
        Returns:
          ...
          and latency.
        """
        delayed_orientation = np.array(self._control_observation[24:28])
    ...
    def ApplyAction(self, angle_commands, velocity_commands, motor_kps = None, motor_kds = None):
        ...
        if self._motor_velocity_limit < np.inf:
            ...
            angle_commands = np.clip(angle_commands, motor_commands_min, motor_commands_max)
            angle_commands = self.correct_pitch_angle(angle_commands)
        ...
        if self._accurate_motor_model_enabled or self._pd_control_enabled:
            ...
            if self._accurate_motor_model_enabled:
                actual_torque, observed_torque = self._motor_model.convert_to_torque(angle_commands, q_true, velocity_commands, qdot_true, motor_kps, motor_kds)
                ...
                print('THIS PART SHOULD NOT RUN')
    ...
    def GetTrueObservation(self):
        ...
        observation.extend(self.GetTrueBaseVelocity())              # 24 - 26
        observation.extend(self.GetTrueBaseOrientation())           # 27 - 30
...
class MinitaurGymEnv(gym.Env):
    ...
    def step(self, action, action_dot):
        ...
        Args:
          ...
          action_dot: A list of desired motor velocities for eight motors
        ...
        observation = self._get_true_observation()
        reward = self._reward()
    ...
    def _reward(self):
        current_base_position = self.minitaur.GetTrueBasePosition()
        ...
        energy_reward = -np.abs(
            np.dot(self.minitaur.GetTrueMotorTorques(),
                self.minitaur.GetTrueMotorVelocities())) * self._time_step
    ...
    def _get_observation_dimension(self):
        ...
        """
        return len(self._get_true_observation())
...
class BoundingController(object):
    ...
    def update(self, t, current_motor_angles, desired_velocity_x):
        self._time = t
        ...
        current_leg_pose = self._motor_angles_to_leg_pose(current_motor_angles)
        if gait_time <= 0.2:
            new_leg_pose = self.FrontStanceController(current_leg_pose)
        elif gait_time <= 0.3:
            new_leg_pose = self.SwingController(current_leg_pose,desired_velocity_x)
        elif gait_time <= 0.511:
            new_leg_pose = self.BackStanceController(current_leg_pose)
        else:
            new_leg_pose = self.SwingController(current_leg_pose,desired_velocity_x)
        new_motor_angles = self._leg_pose_to_motor_angles(new_leg_pose)
        return new_motor_angles

    def FrontStanceController(self, current_leg_pose):
        new_leg_pose = current_leg_pose
        return new_leg_pose

    def BackStanceController(self, current_leg_pose):
        new_leg_pose = current_leg_pose
        return new_leg_pose

    def SwingController(self, current_leg_pose, desired_velocity_x):
        new_leg_pose = current_leg_pose
        return new_leg_pose
...
def main(argv):
    ...
    try:
        ...
        con = BoundingController(env.minitaur, Inputs_Joint_Position[0])
        print('START WALKING')
        num_iter = range(611)
        for i in num_iter:
            ...
            action = con.update(t,action,desired_velocity_x)
            q_true = env.step(action,action_dot)
            # q_true has 31 values every iter, motor angles, motor velocities, motor torque, base quat, base vel
            print('desire joint angles are ', action[0:4])
            print('actual joint angles are ', q_true[0:4])
            base_velocity_x_actual.append(q_true[28])
            base_angle_pitch_actual.append(q_true[32])
            print('true base velocity is ', q_true[28:31])
            motor_flL_pos_desire, motor_flL_pos_actual, motor_flR_pos_desire, motor_flR_pos_actual = collect_data_front_left_position(action, q_true)
            motor_flL_vel_desire, motor_flL_vel_actual, motor_flR_vel_desire, motor_flR_vel_actual = collect_data_front_left_velocity(action_dot, q_true)
            motor_blL_pos_desire, motor_blL_pos_actual, motor_blR_pos_desire, motor_blR_pos_actual = collect_data_back_left_position(action, q_true)
            motor_blL_vel_desire, motor_blL_vel_actual, motor_blR_vel_desire, motor_blR_vel_actual = collect_data_back_left_velocity(action_dot, q_true)
        ...
        plt.subplot(2,1,1)
        print(len(num_iter))
        print(len(base_velocity_x_actual))
        plt.plot(num_iter, base_velocity_x_desire, 'b',
                 num_iter, base_velocity_x_actual,'r')
        plt.subplot(2,1,2)
        plt.plot(num_iter, base_angle_pitch_desire, 'b',
